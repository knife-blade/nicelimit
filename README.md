...
| suchtool.nicelimit.limited-content-type    | 被限制时内容类型（包括：禁止访问、限流）    | text/plain;charset=UTF-8   |
| suchtool.nicelimit.limited-message         | 被限制时提示信息（包括：禁止访问、限流）    | 哎呀，访问量好大，请稍后再试试吧~  |
...
如果suchtool.nicelimit.forbid里的limited-status-code、limited-content-type、limited-message没配置，则取顶层（suchtool.nicelimit.xxx）的配置。
...
如果suchtool.nicelimit.rate-limit里的limited-status-code、limited-content-type、limited-message没配置，则取顶层（suchtool.nicelimit.xxx）的配置。
